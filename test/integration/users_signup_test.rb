...
class UsersSignupTest < UsersSignup
  ...
  test "valid signup information with account activation" do
    ...
    assert_difference 'User.count', 1 do
      post users_path, params: { user: { name:  "valid_user",
...
class AccountActivationTest < UsersSignup
  ...
  test "should not be able to log in with invalid activation token" do
    get edit_account_activation_url("invalid activation token", email: @user.email)
    assert_not is_logged_in?
  end

  test "should not be able to log in with invalid email" do
    get edit_account_activation_url(@user.activation_token, email: "invalid email")
    assert_not is_logged_in?
  end
